...
void solve(){
    ...
    y=y/g;
    x=x/g;
    int xdiv=divide[x].size();
    int ydiv=divide[y].size();
    vll tmp1(xdiv,1e10);
    vll tmp2(ydiv,1e10);
    tmp1[0]=0;
    tmp2[0]=0;
    for(int i=0;i<xdiv;i++){
        for(int j=i+1;j<xdiv;j++){
            if(divide[x][j]%divide[x][i]==0){
                int c=divide[x][j]/divide[x][i];
                if(c<=k){
                    tmp1[j]=min(tmp1[j],tmp1[i]+1);
                }
            }
        }
    }
    for(int i=0;i<ydiv;i++){
        for(int j=i+1;j<ydiv;j++){
            if(divide[y][j]%divide[y][i]==0){
                int c=divide[y][j]/divide[y][i];
                if(c<=k){
                    tmp2[j]=min(tmp2[j],tmp2[i]+1);
                }
            }
        }
    }
    if(tmp1[xdiv-1]==1e10 || tmp2[ydiv-1]==1e10){
        cout<<-1<<endl;
        return;
    }
    cout<<tmp1[xdiv-1]+tmp2[ydiv-1]<<endl;
}
