...
void solve(){
    ...
    vi actual;
    int size=divide[x].size();
    for(int i=0;i<n;i++){
        int idx=lower_bound(all(divide[x]),a[i])-divide[x].begin();
        if(idx!=size && divide[x][idx]==a[i]){
            actual.pb(a[i]);
        }
    }
    if(actual.size()==0){
    ...
    v(bool) can(size,false);
    can[m[actual[0]]]=true;
    int parts=1;
    for(int i=1;i<actual.size();i++){
        v(bool) can2=can;
        can2[m[actual[i]]]=true;
        for(int j=0;j<size;j++){
            if(can[j] && x%(1LL*divide[x][j]*actual[i])==0){
                can2[m[divide[x][j]*actual[i]]]=true;
            }
        }
        can=can2;
        if(can[size-1]){
